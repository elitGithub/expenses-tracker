...
class UsersList
{
    ...
    public function loadUserList(User $user): array
    {
        $query = "SELECT
                  ...
                  LEFT JOIN `{$this->tables['users_table_name']}` AS `creators` ON `creators`.`created_by` = `users`.`user_id`
        ...
        if (!PermissionsManager::isAdmin($user)) {
            ...
            $where = ' AND `roles`.`role_id` IN (' . generateQuestionMarks($roleIds) . ') ';
            $params = $roleIds;
        }

        $query .= $where . ' GROUP BY  `users`.`user_id`';
